...
from .catalog import get_known_model, get_known_models
from .models import ProviderModel


class ProviderModelCreateForm(forms.ModelForm):
    known_model = forms.ChoiceField(
        label="Modelo conhecido",
        required=True,
        widget=forms.Select(attrs={"class": "form-select"}),
    )

    catalog_help_text = (
        "Selecione um modelo conhecido do provider para evitar duplicidade e inconsistencias de catalogo."
    )
    catalog_warning = ""
    selected_known_model = None
    ...
    def __init__(self, *args, **kwargs):
        ...
        known_models = get_known_models(
            selected_provider.slug if selected_provider is not None else None
        )
        ...
        if selected_provider is None:
            self.catalog_help_text = (
                "Selecione um provider para carregar o catalogo conhecido de modelos."
            )
        elif not known_models:
            self.catalog_warning = (
                "Este provider ainda nao possui catalogo conhecido configurado."
            )

        selected_model_key = (
            (self.data.get("known_model") if self.is_bound else None) or ""
        ).strip()
        self.selected_known_model = get_known_model(
            selected_provider.slug if selected_provider else None,
            selected_model_key,
        )
    ...
    def _build_known_model_choices(
        ...
        *,
        ...
        if not known_models:
            return [("", "Nenhum modelo conhecido para este provider")]
        return [("", "Selecione um modelo")] + [
            (model.key, model.label) for model in known_models
        ]
    ...
    def clean(self):
        ...
        known_models = get_known_models(provider.slug)
        if not known_models:
            self.add_error(
                "provider",
                "Este provider ainda nao possui catalogo conhecido configurado.",
        ...
        known_model = get_known_model(provider.slug, model_key)
        if known_model is None:
            self.add_error(
                "known_model",
                "Selecione um modelo conhecido valido para o provider informado.",
            )
            return cleaned_data
